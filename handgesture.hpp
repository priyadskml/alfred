...
class HandGesture{
	public:
		...
		vector<Point> fingertips_pointing;
		int nrOfDefects;
		Rect bRect;
		double bRect_width;
		double bRect_height;
		bool isHand;
		bool detectIfHand();
		bool detectIfHand_sm();
		void initVectors();
		void getFingerNumber(MyImage *m);
		void eleminateDefects(MyImage *m);
		void getFingerTips(MyImage *m);
		int find_pointing_finger(MyImage *m); 
		float perpendicular_distance(Point a,Point b,Point c);
		int directions(MyImage * m);
...
//perpendicular_distance functiom
float HandGesture::perpendicular_distance(Point a,Point b,Point c)
{
	Point d;
	d.x=(a.x+b.x)/2;
	d.y=(a.y+b.y)/2;
	float distance = distanceP2P(d,c);
	return distance;
}
//pointing gesture for camera
int HandGesture::find_pointing_finger(MyImage *m)
{
	int count=0;
	int tolerance = bRect_height/4;
	float angleTol =95;
	...
	{
		Vec4i& v=(*d);
		startidx=v[0]; Point ptStart(contours[cIdx][endidx]);
		endidx=v[1]; Point ptEnd(contours[cIdx][endidx]);
		faridx=v[2]; Point ptFar(contours[cIdx][faridx]);
		//cout<<"heyyy11111"<<endl;
		if(distanceP2P(ptStart,ptFar) > tolerance && distanceP2P(ptEnd,ptFar)>tolerance)
		{
			//cout<<"heyyy"<<endl;
			//cout<<"distance"<<perpendicular_distance(ptStart,ptEnd,ptFar);
			//cout<<"Angle"<<getAngle(ptStart,ptFar,ptEnd)<<endl;
			if(getAngle(ptStart,ptFar,ptEnd) >120 && getAngle(ptStart,ptFar,ptEnd) <140)
			{
				fingertips_pointing.push_back(ptStart);
				fingertips_pointing.push_back(ptEnd);
				fingertips_pointing.push_back(ptFar);
				count++;
			} 
			if(getAngle(ptStart,ptFar,ptEnd)>60 && getAngle(ptStart,ptFar,ptEnd) < 90)
			{
				fingertips_pointing.push_back(ptStart);
				fingertips_pointing.push_back(ptEnd);
				fingertips_pointing.push_back(ptFar);
				count++;
				//cout<<"This is the count that you wanted"<<count<<endl;
			}
			if(getAngle(ptStart,ptFar,ptEnd)>34 && getAngle(ptStart,ptFar,ptEnd) < 46)
			{
				fingertips_pointing.push_back(ptStart);
				fingertips_pointing.push_back(ptEnd);
				fingertips_pointing.push_back(ptFar);
	...
	// 	return 0;
	// int p1 = 5;//fingerTips.size();
	// cout << p1 << endl;
	// if(p1>2)
	// {
	// 	int p2 = p1 -2;
	// 	Point there = fingerTips[p1-1];
	// 	Point here = fingerTips[p2];
	// 	if(there.y > here.y)
	// 		cout << "right" <<endl<<endl;
	// 	else cout << "left" << endl<<endl;
	// }
	if(count == 2 )
	{
		return 1;
...
//directions
int HandGesture::directions(MyImage * m)
{
	int count=0;
	int tolerance = bRect_height/4;
	float angleTol =95;
	vector<Vec4i> newDefects;
	int startidx,endidx,faridx,faridx1;
	vector<Vec4i>::iterator d=defects[cIdx].begin();
	cout<<"Entered"<<endl;
	// while(d!=defects[cIdx].end())
	...
	endidx=v[1]; Point ptEnd(contours[cIdx][endidx]);
	faridx=v[2]; Point ptFar(contours[cIdx][faridx]);
	faridx1=v[3]; Point ptFar1(contours[cIdx][faridx1]);

	if(ptStart.y > ptFar.y + 50)
	{
		cout << "left" << endl <<endl;
		direction = 1;
		return direction;
	}
	else if(ptStart.y +50 < ptFar.y)
	{
		cout << "right" << endl<<endl;
		direction = 0;
		return direction;
...
//anothr function to check ifhand
bool HandGesture::detectIfHand_sm(){
	analyzeContours();
	double h = bRect_height; 
	double w = bRect_width;
	isHand=true;
	if(fingerTips.size() > 8 && fingerTips.size()<3){
		//cout << "I am working" << endl <<endl<<endl;
		isHand=false;
	}else if(h==0 || w == 0){
		isHand=false;
	}else if(h/w > 4 || w/h >4){
